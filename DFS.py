...
if __name__ == "__main__":
    ...
    path, distance = dfs(test_graph, 'A', 'D')
    print(f"Đường đi từ A đến D: {path}")
    print(f"Tổng khoảng cách: {distance}")
    
    print("\nTest DFS (Iterative):")
    path2, distance2 = dfs_iterative(test_graph, 'A', 'D')
    print(f"Đường đi từ A đến D: {path2}")
    print(f"Tổng khoảng cách: {distance2}")
    
    print("\nTất cả đường đi từ A đến D:")
    all_paths = dfs_all_paths(test_graph, 'A', 'D', max_depth=4)
    for i, (path, dist) in enumerate(all_paths, 1):
        print(f"Đường {i}: {' -> '.join(path)} (Khoảng cách: {dist})")
    
    print("\nĐường đi dài nhất từ A đến D:")
    longest, max_dist = dfs_longest_path(test_graph, 'A', 'D')
    print(f"Đường đi: {' -> '.join(longest)} (Khoảng cách: {max_dist})")
    
    print("\nKiểm tra cycle:")
    has_cycle, cycle_path = dfs_cycle_detection(test_graph, 'A')
    print(f"Có cycle: {has_cycle}")
    if has_cycle:
